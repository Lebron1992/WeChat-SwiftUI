...
public final class Strings {
  ...
  private let supportedLocales = ["zh", "en"]

  public init() {}

  // MARK: - Public Functions

  public func fetchStringsByLocale(_ JSONPath: String) throws -> [String: AnyObject]? {
    do {
  ...
  public func localePathsAndContents(
    with rootPath: String,
    stringsByLocale: [String: [String: String]]
  ) -> [(String, String)] {
    ...
    stringsByLocale.forEach { locale, strings in
      ...
      let trueLocale = locale == "zh" ? "zh-Hans" : locale
      let path = "\(rootPath)/\(trueLocale).lproj/Localizable.strings"
...
//      .filter { key in Strings.counts.reduce(true) { $0 && !key.hasSuffix(".\($1)") } }
      .filter { key in Strings.counts.allSatisfy { !key.hasSuffix(".\($0)") } }
      .sorted()
      .forEach { key in
        guard let string = (stringsByLocale["en"]?[key]) else {
